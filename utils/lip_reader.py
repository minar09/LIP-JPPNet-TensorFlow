...
def random_resize_img_labels(image, label, heatmap, resized_h, resized_w):
    ...
    label = tf.image.resize_nearest_neighbor(
        tf.expand_dims(label, 0), new_shape)
    label = tf.squeeze(label, squeeze_dims=[0])
    heatmap = tf.image.resize_nearest_neighbor(
        tf.expand_dims(heatmap, 0), new_shape)
...
def resize_img_labels(image, label, heatmap, resized_h, resized_w):
    ...
    label = tf.image.resize_nearest_neighbor(
        tf.expand_dims(label, 0), new_shape)
    label = tf.squeeze(label, squeeze_dims=[0])
    new_shape = tf.stack([tf.to_int32(resized_h / 8.0),
                          tf.to_int32(resized_w / 8.0)])
    heatmap = tf.image.resize_nearest_neighbor(
        tf.expand_dims(heatmap, 0), new_shape)
    heatmap = tf.squeeze(heatmap, squeeze_dims=[0])
    return img, label, heatmap
...
def random_crop_and_pad_image_and_labels(image, label, heatmap, crop_h, crop_w, ignore_label=255):
    ...
    new_shape = tf.stack(
        [tf.to_int32(crop_h / 8.0), tf.to_int32(crop_w / 8.0)])
    heatmap = tf.image.resize_nearest_neighbor(
        tf.expand_dims(heatmap_crop, 0), new_shape)
    heatmap = tf.squeeze(heatmap, squeeze_dims=[0])
...
def read_pose_list(data_dir, data_id_list):
    ...
    for line in f:
        pose = line.strip("\n")
        poses.append(data_dir + '/heatmaps/' + pose)
    return poses
...
def read_images_from_disk(input_queue, input_size, random_scale, random_mirror=False):
    ...
    # Get all pose heatmaps for this image and concatenate them
    ...
    if input_size is not None:
        ...
        if random_mirror:
            img, label, heatmap = image_mirroring(
                img, label, label_rev, heatmap, heatmap_rev)

        # Randomly resize the images and labels.
        if random_scale:
            img, label, heatmap = random_resize_img_labels(
                img, label, heatmap, h, w)
            # Random scale must be followed by crop to create fixed size
            img, label, heatmap = random_crop_and_pad_image_and_labels(
                img, label, heatmap, h, w, IGNORE_LABEL)
        else:
...
class LIPReader(object):
    ...
    def __init__(self, data_dir, data_list, data_id_list, input_size, random_scale,
        ...
        self.image_list, self.label_list, self.label_rev_list = read_labeled_image_list(
        ...
        self.queue = tf.train.slice_input_producer(
            [self.images, self.labels, self.labels_rev, self.poses], shuffle=shuffle)
        self.image, self.label, self.heatmap = read_images_from_disk(
    ...
    def dequeue(self, num_elements):
        ...
        image_batch, label_batch, heatmap_batch = tf.train.batch(
            [self.image, self.label, self.heatmap], num_elements)
